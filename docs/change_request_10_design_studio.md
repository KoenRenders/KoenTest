...
| Q24 | 19 Sep | Koen's HDEV round 3 (19 Sep, evening): "we zijn te fel doorgeslagen". | **Cut the design text to three fields** — subtitle bar, handwritten line, "Omschrijving anders" (prefilled with the activity's description, #1016; stored only when it differs). Gone: deviating title, line breaks, recurrence line, price, welcome line, kicker tick, practical, programme. **"Iedereen welkom" is always on the poster**, "Enkel leden" when the activity is members-only. **Title lines share one size**, at the top over the full width; **the lockup moves into the band** bottom-left (Koen: "gewoon doen"). **One picture fills the right column**; the third picture takes the bottom-left spot unless sponsor logos do. **Three presets**: eenvoudig (one big picture over the full width, a few facts — the Bowlen poster, half of what the unit makes), beeld, tekst — explained in the picker. **AI**: a style choice (line drawing / flat colours — "voeg kleuren toe" was ignored because the one suffix forbade colour) and "wat wil je anders?" on a variant, which redoes it with that picture as reference (6 credits). |
| Q23 | 19 Sep | Does the pilot need the "Ontwerpen" jump-link on `/admin/activiteiten/{id}` (activities module)? (CLI) | Koen, 19 Sep: **left out of the pilot.** The Design Studio is a menu line of its own (Communicatie → Design Studio → "+ Nieuw ontwerp" → choose the activity); the link is a shortcut, not a function. Add later if it is missed. |
