...
`design-system.html` (met #783 verwijderd) en de normatieve delen van
`ui-conventies.md` zijn daarin
...
**ORM-gebruik en facade-inhoud staan overal op ✅.** Geen enkele UI-module raakt de
sessie nog aan — geen query, geen commit; de transactiegrens ligt in de services.
En geen enkele facade laat nog een ORM-klasse of routerfunctie naar een scherm
door.
...
bestelregels). Die verhuizen in een eigen batch, mét tests bij de verhuizing — een
facade-doorgang zou de router alleen verbergen.

De kolom *view-model* volgt de omzetting naar `ViewModel`-dataclasses (#643): de
drie referentieschermen zijn om, de rest volgt per batch. Een leeg vakje daar
betekent dat het scherm zijn context nog als dict doorgeeft.
