...
| 3b | An activity title in the system prompt (#1126) carries a member's name past the name check, which this prompt is exempt from | The title goes through the same name scrubber as the typed question (`_activity_label`): a member's name becomes that household's token before the prompt is built. The exemption's premise — no stored person name in this prompt — is enforced rather than assumed | `test_raakje_noemt_de_activiteit_1126.py`, red-proved by skipping the scrub |
