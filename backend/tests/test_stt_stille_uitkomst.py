...
**Deze tests zijn tegelijk de gedragsproef van #777.** Conftest draait alembic in
dit proces, en tot #777 zette `alembic/env.py` daarbij elke bestaande logger uit —
`caplog` las dan een lege lijst en elke assertie hieronder zou niets meten en tóch
groen staan. Ze lezen daarom bewust via `caplog` en niet via een eigen handler: draait
die parameter terug, dan vallen ze alle vier om.
...
def test_geval_1_de_provider_weigert(client, monkeypatch, caplog):
    """Een expliciete weigering komt als waarschuwing boven, mét de snelheid — want
    de snelheid is dan juist de verdachte."""
    with caplog.at_level(logging.INFO, logger=stt_mod.logger.name):
